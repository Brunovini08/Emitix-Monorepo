export class ICMSUFDest {
  ...
  public validateOrThrow() {
    if (typeof this.vBCUFDest !== 'number' || this.vBCUFDest < 0) {
      throw new Error('Valor da Base de Cáculo do ICMS na UF do destinatário (vBCUFDest) é obrigatório e deve ser um número não negativo.');
    }

    if (this.vBCFCPUFDest !== null && (typeof this.vBCFCPUFDest !== 'number' || this.vBCFCPUFDest < 0)) {
      throw new Error('Valor da Base de Cálculo do FCP na UF do destinatário (vBCFCPUFDest) deve ser um número não negativo, se informado.');
    }

    if (this.pFCPUFDest !== null && (typeof this.pFCPUFDest !== 'number' || this.pFCPUFDest < 0 || this.pFCPUFDest > 100)) {
      throw new Error('Percentual adicional FCP (pFCPUFDest) deve ser um número entre 0 e 100, se informado.');
    }

    if (typeof this.pICMSUFDest !== 'number' || this.pICMSUFDest < 0 || this.pICMSUFDest > 100) {
      throw new Error('Alíquota adotada nas operações internas na UF do destinatário (pICMSUFDest) é obrigatória e deve ser um número entre 0 e 100.');
    }

    const allowedPICMSInter = ['4.00', '7.00', '12.00'];
    if (typeof this.pICMSInter !== 'string' || !allowedPICMSInter.includes(this.pICMSInter)) {
      throw new Error('Alíquota interestadual das UF envolvidas (pICMSInter) é obrigatória e deve ser "4.00", "7.00" ou "12.00".');
    }

    if (typeof this.pICMSInterPart !== 'number' || this.pICMSInterPart < 0 || this.pICMSInterPart > 100) {
      throw new Error('Percentual de partilha para a UF do destinatário (pICMSInterPart) é obrigatório e deve ser um número entre 0 e 100.');
    }

    if (this.vFCPUFDest !== null && (typeof this.vFCPUFDest !== 'number' || this.vFCPUFDest < 0)) {
      throw new Error('Valor do FCP na UF do destinatário (vFCPUFDest) deve ser um número não negativo, se informado.');
    }

    if (typeof this.vICMSUFDest !== 'number' || this.vICMSUFDest < 0) {
      throw new Error('Valor do ICMS de partilha para a UF do destinatário (vICMSUFDest) é obrigatório e deve ser um número não negativo.');
    }

    if (typeof this.vICMSUFRemt !== 'number' || this.vICMSUFRemt < 0) {
      throw new Error('Valor do ICMS de partilha para a UF do remetente (vICMSUFRemt) é obrigatório e deve ser um número não negativo.');
    }
  }
