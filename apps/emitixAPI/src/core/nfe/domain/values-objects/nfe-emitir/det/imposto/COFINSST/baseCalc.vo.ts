...
export class BaseCalc {
  ...
  public validateOrThrow() {
    if (typeof this.vBC !== 'number' || this.vBC < 0) {
      throw new DomainError('Valor da BC do COFINS ST (vBC) é obrigatório e deve ser um número não negativo.');
    }

    if (typeof this.pCOFINS !== 'number' || this.pCOFINS < 0 || this.pCOFINS > 100) {
      throw new DomainError('Alíquota do COFINS ST (em percentual) (pCOFINS) é obrigatória e deve ser um número entre 0 e 100.');
    }
  }
