...
export class ICMSSN201 {
  ...
  public validateOrThrow() {
    ...
    if (this.CSOSN !== '201') {
      throw new DomainError('CSOSN para ICMS201 deve ser obrigatoriamente "201".');
    }
    ...
    if (this.pFCPST !== null && (typeof this.pFCPST !== 'string' || this.pFCPST.trim() === '')) {
      throw new DomainError('Alíquota do ICMS FCP (pFCPST) deve ser um número entre 0 e 100, se informado.');
    }

    if (this.vFCPST !== null && (typeof this.vFCPST !== 'string' || this.vFCPST.trim() === '')) {
      throw new DomainError('Valor do ICMS FCP (vFCPST) deve ser um número não negativo, se informado.');
    }

    if (this.pCredSN !== null && (typeof this.pCredSN !== 'string' || this.pCredSN.trim() === '')) {
      throw new DomainError('Alíquota do ICMS ST (pCredSN) deve ser um número entre 0 e 100, se informado.');
    }

    if (this.vCredICMSSN !== null && (typeof this.vCredICMSSN !== 'string' || this.vCredICMSSN.trim() === '')) {
      throw new DomainError('Valor do ICMS ST (vCredICMSSN) deve ser um número não negativo, se informado.');
    }
  }
