...
export class ICMS15 {
  ...
  public validateOrThrow() {
    if (this.orig === undefined || !(Object.values(TorigEnum).includes(this.orig))) {
      throw new DomainError(`Origem da mercadoria (orig) é obrigatória e deve ser um dos valores válidos: 0, 1, 2.`);
    }
    ...
    if (this.CST !== '15') {
      throw new DomainError('CST para ICMS15 deve ser obrigatoriamente "15".');
    }

    if (this.qBCMono !== undefined && (typeof this.qBCMono !== 'string' || this.qBCMono.trim() === '')) {
      throw new DomainError('Quantidade tributada (qBCMono) deve ser um número não negativo, se informada.');
    }

    if (typeof this.adRemICMS !== 'string' || this.adRemICMS.trim() === '') {
      throw new DomainError('Alíquota ad rem do imposto (adRemICMS) é obrigatória e deve ser um número positivo.');
    }

    if (typeof this.vICMSMono !== 'string' || this.vICMSMono.trim() === '') {
      throw new DomainError('Valor do ICMS próprio (vICMSMono) é obrigatório e deve ser um número não negativo.');
    }

    if (this.qBCMonoReten !== undefined && (typeof this.qBCMonoReten !== 'string' || this.qBCMonoReten.trim() === '')) {
      throw new DomainError('Quantidade tributada sujeita a retenção (qBCMonoReten) deve ser um número não negativo, se informada.');
    }

    if (this.adRemICMSReten !== undefined && (typeof this.adRemICMSReten !== 'string' || this.adRemICMSReten.trim() === '')) {
      throw new DomainError('Alíquota ad rem do imposto com retenção (adRemICMSReten) deve ser um número positivo, se informada.');
    }

    if (this.vICMSMonoReten !== undefined && (typeof this.vICMSMonoReten !== 'string' || this.vICMSMonoReten.trim() === '')) {
      throw new DomainError('Valor do ICMS com retenção (vICMSMonoReten) deve ser um número não negativo, se informado.');
    }

    if (this.pRedAdRem !== undefined && (typeof this.pRedAdRem !== 'string' || this.pRedAdRem.trim() === '')) { 
      throw new DomainError('Percentual de redução do valor da alíquota ad rem do ICMS (pRedAdRem) deve ser um número entre 0 e 100, se informado.');
    }
    ...
    if (this.motRedAdRem !== undefined && typeof this.motRedAdRem !== 'string' && !allowedMotRedAdRem.includes(this.motRedAdRem)) {
        throw new DomainError(`Motivo da redução do adrem (motRedAdRem) deve ser '1' ou '9', se informado.`);
    }
  }
