...
export class PISAliq {
  ...
  public validateOrThrow() {
    ...
    if (typeof this.vBC !== 'number' || this.vBC < 0) {
      throw new DomainError('Valor da BC do PIS (vBC) é obrigatório e deve ser um número não negativo.');
    }

    if (typeof this.pPIS !== 'number' || this.pPIS < 0 || this.pPIS > 100) {
      throw new DomainError('Alíquota do PIS (em percentual) (pPIS) é obrigatória e deve ser um número entre 0 e 100.');
    }
