...
export class GCred {
  ...
  public validateOrThrow(): void {
    if (!this.cCredPresumido || this.cCredPresumido.trim() === '') {
      throw new DomainError('Código do Crédito Presumido (cCredPresumido) é obrigatório.');
    }
    if (typeof this.pCredPresumido !== 'number' || this.pCredPresumido < 0) {
      throw new DomainError('Percentual do Crédito Presumido (pCredPresumido) deve ser um número não negativo.');
    }
    if (typeof this.vCredPresumido !== 'number' || this.vCredPresumido < 0) {
      throw new DomainError('Valor do Crédito Presumido (vCredPresumido) deve ser um número não negativo.');
    }
  }
