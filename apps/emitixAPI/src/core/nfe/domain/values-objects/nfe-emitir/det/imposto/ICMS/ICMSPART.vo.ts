...
export class ICMSPart {
  ...
  public validateOrThrow() {
    ...
    if (this.CST !== '90') {
      throw new DomainError('CST para ICMSPART deve ser obrigatoriamente "90".');
    }
    ...
    if (typeof this.pRedBC !== 'string' || this.pRedBC.trim() === '') {
      throw new DomainError('Percentual de redução da BC (pRedBC) é obrigatório e deve ser um número entre 0 e 100.');
    }
    ...
    if (this.pBCOp !== null && (typeof this.pBCOp !== 'string' || this.pBCOp.trim() === '')) {
      throw new DomainError('Percentual da BC da Operação Própria (pBCOp) deve ser um número entre 0 e 100, se informado.');
    }
    ...
    if (this.UFST !== null && (typeof this.UFST !== 'string' || !allowedUF.includes(this.UFST))) {
        throw new DomainError(`UF do ICMS ST (UFST) deve ser um dos valores de UF válidos, se informado.`);
    }
  }
