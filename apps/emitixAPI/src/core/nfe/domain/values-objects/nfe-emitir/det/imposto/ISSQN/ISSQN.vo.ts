...
export class ISSQN {
  ...
  public validateOrThrow() {
    if (typeof this.vBC !== 'number' || this.vBC < 0) {
      throw new DomainError('Valor da BC do ISSQN (vBC) é obrigatório e deve ser um número não negativo.');
    }

    if (typeof this.vAliq !== 'number' || this.vAliq < 0 || this.vAliq > 100) {
      throw new DomainError('Alíquota do ISSQN (vAliq) é obrigatória e deve ser um número entre 0 e 100.');
    }

    if (typeof this.vISSQN !== 'number' || this.vISSQN < 0) {
      throw new DomainError('Valor do ISSQN (vISSQN) é obrigatório e deve ser um número não negativo.');
    }

    if (typeof this.cMunFG !== 'string' || this.cMunFG.length === 0) {
      throw new DomainError('Município de ocorrência do fato gerador do ISSQN (cMunFG) é obrigatório.');
    }

    if (typeof this.cListServ !== 'string' || this.cListServ.length === 0) {
      throw new DomainError('Item da lista de serviços (cListServ) é obrigatório.');
    }

    if (this.vDeducao !== null && (typeof this.vDeducao !== 'number' || this.vDeducao < 0)) {
      throw new DomainError('Valor da dedução (vDeducao) deve ser um número não negativo, se informado.');
    }

    if (this.vOutro !== null && (typeof this.vOutro !== 'number' || this.vOutro < 0)) {
      throw new DomainError('Outros valores (vOutro) deve ser um número não negativo, se informado.');
    }

    if (this.vDescIncond !== null && (typeof this.vDescIncond !== 'number' || this.vDescIncond < 0)) {
      throw new DomainError('Valor do desconto incondicionado (vDescIncond) deve ser um número não negativo, se informado.');
    }

    if (this.vDescCond !== null && (typeof this.vDescCond !== 'number' || this.vDescCond < 0)) {
      throw new DomainError('Valor do desconto condicionado (vDescCond) deve ser um número não negativo, se informado.');
    }

    if (this.vISSRet !== null && (typeof this.vISSRet !== 'number' || this.vISSRet < 0)) {
      throw new DomainError('Valor do ISSQN retido (vISSRet) deve ser um número não negativo, se informado.');
    }
    ...
    if (this.cServico !== null && (typeof this.cServico !== 'string' || this.cServico.length < 1 || this.cServico.length > 20)) {
      throw new DomainError('Código do serviço (cServico) deve ser uma string com 1 a 20 caracteres, se informado.');
    }

    if (this.cMun !== null && (typeof this.cMun !== 'string' || this.cMun.length === 0)) {
      throw new DomainError('Código do município (cMun) deve ser uma string válida, se informado.');
    }

    if (this.cPais !== null && (typeof this.cPais !== 'string' || !/^[0-9]{1,4}$/.test(this.cPais))) {
      throw new DomainError('Código do país (cPais) deve ser uma string de 1 a 4 dígitos numéricos, se informado.');
    }

    if (this.nProcesso !== null && (typeof this.nProcesso !== 'string' || this.nProcesso.length < 1 || this.nProcesso.length > 30)) {
      throw new DomainError('Número do processo (nProcesso) deve ser uma string com 1 a 30 caracteres, se informado.');
    }

    const allowedIndIncentivo = ['1', '2'];
    if (typeof this.indIncentivo !== 'string' || !allowedIndIncentivo.includes(this.indIncentivo)) {
      throw new DomainError('Indicador de Incentivo Fiscal (indIncentivo) é obrigatório e deve ser "1" (Sim) ou "2" (Não).');
    }
  }
