...
import { enviNFeGen } from "src/resources/common/utils/generate/enviNFeGen.util";
import { extractCNPJFromSubject } from "src/resources/common/utils/validate/getCnpjFromCertificate.util";
import { validateCertificate } from "src/resources/common/utils/validate/validateCertificate.util";
import { validateXmlXsd } from "src/resources/common/utils/validate/validateXmlXsd.util";
...
import { generateAccessKeyToEvent, generateAccessKeyToInutNfe } from "src/resources/common/utils/generate/generateAccessKey.util";
...
import { TEnvConsCad } from "src/core/nfe/reusable/types/complex_types/TCons/TEnvConsCad";
...
import { signedEventXml } from "src/resources/common/utils/common/signedEvent.util";
...
export class NotaService {
  ...
  async emitir(
    ...
    if (valide === 'Certificado ainda não é válido.')
      ...
      const sign = signedXml(
        ...
        file,
        certPassword,
        createXML.chave_acesso ?? (() => { throw new BadRequestException('Chave de acesso não foi gerada'); })(),
        'infNFe'
      );
      if (sign !== undefined) {
  ...
  async inutilizarNFe(
    body: TEnvInutNfe, file: Base64, certPassword: string, nUrl: number, typeDocument: string,
    cert: any, privateKey: any,
  ) {
    const valide = validateCertificate(cert)
